...
def check_phases(sma200_pct, rsi, qqq_pct, vix, fg, ret5d, cape):
    ...
    elif v0_others and not v0_cape:
        # CAPE 제외 나머지 V0 조건 전부 충족 — CAPE 수동 확인 필수
        alerts.append(("⚠️ V0 CAPE 확인 필수", f"VIX·RSI·이격도 조건 충족 — CAPE 수동 확인 후 V0 전환 판단 (현재값: {cape})", "#eab308"))
...
def send_kakao(text):
    ...
    data = {
        "template_object": json.dumps({
            "object_type": "text",
            "text": text,
            "link": {"web_url": "https://www.multpl.com/shiller-pe"}
        })
    }
